...
T_OFFSET_UNIT_423 = 0.10  # increment unit for OFFSET_CORR register in K, FW >= 4.2.3
OBJ_TEMP_FACTOR_UNIT = 0.01  # unitless object temperature correction factor
...
DEFAULT_CTRL_STAT = {
    'FRAME_MODE': 0x20,
    'STATUS':     0x00,
    'FRAME_RATE': 0x03, # was 4 for bobocat
    'POWER_DOWN_1': 0x00,
    'POWER_DOWN_2': 0x02,
#    "SENS_FACTOR" : 0x64,  # read from camera module; ideally 0x64 = 1.00
    'EMISSIVITY': 0x5F,
#    'OFFSET_CORR': 0x00, module dependent
    'FILTER_1': 0x01,
    'FILTER_1_LSB': 0x0A,
    'FILTER_1_MSB': 0x00,
    'FILTER_2': 0x03,
...
regmap = {
    ...
    # SSEC
    'COMP_CTRL'     : 0x02,  # RW Temperature Error Compensation Control
    "COMPPAR_P0"    : 0x03,  # RW temperature error compensation parameter p0
    "COMPPAR_P1"    : 0x05,  # RW temperature error compensation parameter p1
    "COMPPAR_P2"    : 0x07,  # RW temperature error compensation parameter p2
    "COMPPAR_P3"    : 0x09,  # RW temperature error compensation parameter p3
    "COMPPAR_P4"    : 0x0B,  # RW temperature error compensation parameter p4
    "COMPPAR_P5"    : 0x0D,  # RW temperature error compensation parameter p5
    # DTC
    "COMPPAR_P6"    : 0x0F,  # RW temperature error compensation parameter p6
    "COMPPAR_P7"    : 0x11,  # RW temperature error compensation parameter p7
    "COMPPAR_P8"    : 0x13,  # RW temperature error compensation parameter p8
    "COMPPAR_P9"    : 0x15,  # RW temperature error compensation parameter p9
    # OCEC
    "COMPPAR_P10"   : 0x17,  # RW temperature error compensation parameter p10
    "COMPPAR_P11"   : 0x1A,  # RW temperature error compensation parameter p11
    #
    "SPI_RETRY"     : 0x19,  # SPI retry mechanism control
    #
    #
    "EVK_ID"        : 0xA5,  # Check EVK ID, (jig only; reads 0xFF on other boards)
    #
    ...
    "SENXOR_TYPE"   : 0xBA,  # R  Type of the attached camera SenXor
    "MODULE_TYPE"   : 0xBB,  # R  Type of the attached camera module
    "LUT_SELECT"    : 0xBC,  # RW LUT selection

    "OFFS_CALIB"    : 0xC0,  # RW Offset Calibration
    "CALIB_CTRL"    : 0xC1,  # RW Calibration settings control flags
    "SENS_FACTOR"   : 0xC2,  # RW Sensitivity factor (float, unitless)
    "BLIND_CALIB"   : 0xC5,  # RW Blind calibration settings
    "EMISSIVITY"    : 0xCA,  # RW Emissivity value for conversion to [K]
    "OFFSET_CORR"   : 0xCB,  # RW Temperature offset correction per frame
    "OBJ_TEMP_FACTOR" : 0xCD,  # RW Object Temperature Factor per frame
    'FILTER_1'      : 0xD0,  # RW Control of filter 1 (temporal)
    'FILTER_1_LSB'  : 0xD1,  # RW Settings of filter 1 (temporal LSB)
    'FILTER_1_MSB'  : 0xD2,  # RW Settings of filter 1 (temporal MSB)
    'FILTER_2'      : 0x20,  # RW Settings of filter 2 (STARK)
    'FILTER_3'      : 0x30,  # RW Settings of filter 2 (median)
    'MMS_CTRL'      : 0x25,  # RW min/max stabilization
...
MI48_COMP_CTRL     = 0x02  # RW Control the on/off flags for temperature compensation
# SSEC
MI48_COMPPAR_P0    = 0x03  # RW temperature error compensation parameter p0
MI48_COMPPAR_P1    = 0x05  # RW temperature error compensation parameter p1
MI48_COMPPAR_P2    = 0x07  # RW temperature error compensation parameter p2
MI48_COMPPAR_P3    = 0x09  # RW temperature error compensation parameter p3
MI48_COMPPAR_P4    = 0x0B  # RW temperature error compensation parameter p4
MI48_COMPPAR_P5    = 0x0D  # RW temperature error compensation parameter p5
# DTC
MI48_COMPPAR_P6    = 0x0F  # RW temperature error compensation parameter p6
MI48_COMPPAR_P7    = 0x11  # RW temperature error compensation parameter p7
MI48_COMPPAR_P8    = 0x13  # RW temperature error compensation parameter p8
MI48_COMPPAR_P9    = 0x15  # RW temperature error compensation parameter p9
# OCEC
MI48_COMPPAR_P10   = 0x17  # RW temperature error compensation parameter p10
MI48_COMPPAR_P11   = 0x1A  # RW temperature error compensation parameter p11

MI48_SPI_RETRY     = 0x19  # SPI retry control
...
MI48_STATUS        = 0xB6  # R  Status of the attached camera module and MI48 interface
MI48_POWER_DOWN_2  = 0xB7  # RW Control of power down parameters
MI48_SENXOR_TYPE   = 0xBA  # R  Type of the attached camera SenXor
MI48_MODULE_TYPE   = 0xBB  # R  Type of the attached camera module
MI48_LUT_SELECT    = 0xBC  # RW LUT selection
MI48_OFFS_CALIB    = 0xC0  # RW Offset Calibration
MI48_CALIB_CTRL    = 0xC1  # RW Calibration settings control flags
MI48_SENS_FACTOR   = 0xC2  # RW Sensitivity factor, unitless, float
MI48_BLIND_CALIB   = 0xC5  # RW Blind calibration settings
MI48_EMISSIVITY    = 0xCA  # RW Emissivity value for conversion to temperature
MI48_OFFSET_CORR   = 0xCB  # RW Temperature offset correction for the entire data frame
MI48_OBJ_TEMP_FACTOR = 0xCD  # RW Object Temperature FActor per frame
MI48_FILTER_1      = 0xD0  # RW Control of filter 1 (temporal)
MI48_FILTER_1_LSB  = 0xD1  # RW Settings of filter 1 (temporal LSB)
MI48_FILTER_1_MSB  = 0xD2  # RW Settings of filter 1 (temporal MSB)
MI48_FILTER_2      = 0x20  # RW Settings of filter 2 (STARK)
MI48_FILTER_3      = 0x30  # RW Settings of filter 2 (Median)
MI48_MMS_CTRL      = 0x25  # RW Settings of Min/Max Stabilization
...
CONTINUOUS_STREAM = 0x02    # with header containing info on compensation algos
READOUT_MODE = 0x1C         # bits 4-2
NO_HEADER = 0x20            # skip header HEADER data
DATA_TYPE_RAW_ADC = 0x80    # output raw ADC codes (after compensation & calibration)
...
SENXOR_NAME = {
    ...
    3: 'MI0802',
    4: 'MI0802',
    6: 'MI1603', # panther, engineering samples
}
...
class MI48:
    ...
    MI48xx abstraction
    """
    def __init__(self, interfaces:list, fps=None, name="MI48", reset_handler=None,
                 parse_header=True, data_ready=None, data_type='temperature',
                 reset_module=False, verbose=True, allow_invalid_sn=False):
        """Initialise with a serial port"""
        # logging stuff
        self.verbose = verbose
        # when more than one units connected, name serves to distinguish them
        # for USB - can assign comport, for example
        self.name = name
        # assign name to logger
        ...
        # reset handler and data_ready are functions needed for 
        # SPI/I2C on embedded systems, R-Pi, etc. to allow the host to 
        # drive the HW Reset line of the MI48, or to read the HW DATA_READY
        # output from the MI48
        self.reset = reset_handler
        if self.reset is not None: self.reset()
        if data_ready is not None: self.data_ready = data_ready

        # connect to MI48 and module
        status, mode = self.bootup(powerup=reset_module, verbose=self.verbose)

        # check if mi48 is left streaming (e.g. due to bad exit before)
        mode = self.get_mode()
...
#            # if the MI48 is streaming, it's obviously booted up before
        ...
        # may need to handle ValueError from above call:
            ...
            # known problem on bridge+core bord (bobcat)
        ...
        if bootup_error:
            status, mode = self.error_handler(status, mode, verbose=self.verbose)
        ...
        # when working with the jig, there seems to be only one way to check if
        # there is a module connected in a given socket - by checking that the SN
        # is not all 0's. There are cases where we may need to work with a 0 SN.
        # in that case, we must use `allow_invalid_sn=True`
        self.module_connected = True
        if self.camera_info['SN'] == 'SN' +\
                            ''.join(['00' for i in range(MI48_SENXOR_ID_LEN)]):
            self.log(logging.ERROR, 'INVALID SenXor Serial Number:'\
                                   f'{self.camera_info["SN"]}')
            if not allow_invalid_sn:
                self.log(logging.CRITICAL,
                         'Refusing to Connect. Try allow_invalid_sn=True if appropriate')
        ...
        if fps is not None:
            self.set_fps(fps)
        # set the type of the returned data
        self.data_type = data_type
        self.temperature_units = 'Celsius'
        # set the appropriate unit for temperature offset; FW dependent
        self.temperature_offset_unit = T_OFFSET_UNIT_423
        if int(''.join(self.fw_version.split('.'))) < 423:
            self.temperature_offset_unit = T_OFFSET_UNIT
        self.otf_unit = OBJ_TEMP_FACTOR_UNIT # for certain applications with jig boards,
                                             # header is different
        # disable parsing if requested during initialization
        self.parse_header = parse_header
        self.mCFilter = None

    def get_fpa_shape(self, chip, module):
        """
        Return the appropriate ncols, nrows of the FPA.
        This is typically defined by the chip (senxor_type)
        But certain exceptions exist, depending on lens, where
        module type defines the FPA resolution.
        """
        # determine the base resolution from the chip
        fpa_shape = FPA_SHAPE_SENXOR[chip]
        # correct for lens-defined resolution if needed
        fpa_shape = FPA_SHAPE_MODULE.get(module, fpa_shape)
        return fpa_shape

    def bootup(self, verbose=True, powerup=False):
        ...
        while boot_in_progress:
            status = self.get_status(verbose=self.verbose)
            mode = self.get_mode(verbose=self.verbose)
            boot_in_progress = status & BOOTING_UP
            time.sleep(timeout)
        t1 = time.monotonic()
    ...
    def error_handler(self, status, mode, verbose=True):
    ...
    def read(self):
        """
        Read a data frame; return the temperature data array or (data, header).

        The returned data is a 1D array, and its data type depends on
        `self.data_type` and on `self.temperature_units`.
        For `raw_adc` or `temperature`in dK, the output is np.uint16.
        For output in Celsius or Fahrenheit, the output is np.float16.
        Header is uint16 array or a dictionary (if `self.parse_header` is True).
        """
        ...
        if self.capture_no_header:
            ...
            _crc = crc16(data)
            if self.parse_header and not header['crc'] == hex(_crc):
                self.crc_error = True
                self.log(logging.ERROR, 'Frame CRC error. '+
                    'Header CRC: {}, Data CRC: {}'.\
                    format(header['crc'], hex(_crc)))
            if self.mCFilter is not None:
                self.mCFilter.CheckSenxor(data, header['iplock1'])
        # Once we have done the CRC check, convert to degrees C
        # unless raw numbers are requested
        if self.data_type == 'raw_adc':
            return data, header
        else:
            if self.temperature_units in ['Celsius', 'C']:
                data = data / 10. + KELVIN_0
                return data.astype(np.float16), header
            if self.temperature_units in ['Fahrenheit', 'F']:
                data = (data / 10. + KELVIN_0) * (9./5) + 32
                return data.astype(np.float16), header
            if self.temperature_units in ['dK', 'deci-Kelvin', 'deciKelvin']:
                return data, header
            return data, header

    def set_data_type(self, data_type):
        """
        Set data type of the unit, from ['raw_adc', 'temperature']
        """
        assert data_type in ['raw_adc', 'temperature']
        self.data_type = data_type

    def set_temperature_units(self, units):
        """
        Set temperature units from deciKelvin, Celcius, Fahrenheit.
        The python data type of the return from `self.read()` is:
            `np.uint16`, for dK, `np.float16`, for C, and F.
        """
        assert units in ['dK', 'deci-Kelvin', 'deciKelvin', 'Celsius', 'C',
                         'Fahrenheit', 'F']
        self.temperature_units = units

    def set_c_filters(self, dll_filter):
        """
        Provide an instance of a dll_filter class.
        """
        self.mCFilter = dll_filter

    def get_c_filters(self):
        """
        Report the settings of the assigned dll_filter instance.
        """
        return self.mCFilter

    def get_evk_socket_id(self):
        """
        Return the position of the socket on the jig.
        Enumeration is from 1 to 42, (1 to 25 if old jig).
        """
    ...
    def get_mode(self, verbose=True):
    ...
    def get_offset_corr_K(self):
        ...
        """
        unit = self.temperature_offset_unit
    ...
    def get_otf(self):
        unit = self.otf_unit
        n = regval = self.regread(MI48_OBJ_TEMP_FACTOR)
        if n < 128:
            return n * unit
        else:
            return (n - 256) * unit
    ...
    def get_filter_1(self):
        """Return the control flags of Temporal Register"""
        ctrl = self.regread('FILTER_1')
        lsb = self.regread('FILTER_1_LSB')
        msb = self.regread('FILTER_1_MSB')
        strength = (msb << 8) + lsb
        return ctrl, strength

    def get_filter_2(self):
        """Return the settings of the STARK filter"""
        return self.regread('FILTER_2')

    def get_filter_3(self):
        """Return the settings the Median filter"""
        return self.regread('FILTER_3')

    def get_camera_info(self, update=True):
        ...
        res['NAME'] = self.name
        res['SENXOR_TYPE'] = self.get_senxor_type()
        ...
        res['SN'] = 'SN'+uid_hex
        res['Wafer_Number'] = int(uid_hex[-5:], base=16) // 1100
        res['FW_VERSION'] = self.get_fw_version()
        self.senxor_type = res['SENXOR_TYPE']
        self.module_type = res['MODULE_TYPE']
        self.senxor_name = SENXOR_NAME[self.senxor_type]
        self.fpa_shape = self.get_fpa_shape(self.senxor_type, self.module_type)
        self.cols = self.fpa_shape[0]
        self.rows = self.fpa_shape[1]
        # synnonyms
        self.ncols = self.fpa_shape[0]
        self.nrows = self.fpa_shape[1]
        # deal with non-conventional number of columns in the header of cougar-50
        self.ncols_header = self.ncols
        if self.module_type == 22:
            # for minicougar-50 the header has the length of the original row, i.e. 80
            self.ncols_header = 80
        #
    ...
    def check_ctrl_stat_regs(self, expect=None):
        ...
        for reg, val in res.items():
            ...
            if reg in expect.keys():
                _exp = expect.get(reg, None)
                if val != _exp and _exp is not None:
    ...
    def get_max_fps(self):
        ...
        This is not accurate at all: depends on FW filter options, compensation,
        MCU, senxor_type, etc.
        """
        ...
        #       and the camera type.
        if self.senxor_type in [0,1]:
            maxfps = 25.5  # this is true for Bobcat with latest MI48Ax
            return maxfps
        if self.senxor_type in [2]:
            maxfps = 28.57 # lynx
            return maxfps
        # override for minicougar
        if self.module_type in [19, 20, 21, 22]:
            # minicougar mass-pro: IR40, 41, 47
            maxfps = 30.0
    ...
    def enable_filter(self, f1=False, f2=False, f3=False, f3_ks_5=False):
        """
        Enable filters: f1-temporal, f2-STARK, f3-median

        Assume settings for f1 and f2 are done in anvance, if not default.
        """
        if f1:
            self.regwrite('FILTER_1', 0x03)
        else:
            self.regwrite('FILTER_1', 0x00)
        if f2:
            self.regwrite('FILTER_2', 0x03)
        else:
            self.regwrite('FILTER_2', 0x00)
        if f3:
            self.regwrite('FILTER_2', 0x01)
        else:
            self.regwrite('FILTER_2', 0x00)
        return None
    ...
    def set_offset_corr(self, offset_in_Kelvin):
        """
        Add an offset to the entire frame in Kelvin.

        A positive value of `offset_in_Kelvin` will raise the readout temperature
        of all pixels by the desired amount.
        A negative value will correspondingly reduce it.
        """
        assert offset_in_Kelvin <= 127 * self.temperature_offset_unit and\
               offset_in_Kelvin >= -127 * self.temperature_offset_unit
        n = int(round(offset_in_Kelvin / self.temperature_offset_unit))
        if n < 0:
            regval = 256 - abs(n)
        else:
            regval = n
    ...
    def set_otf(self, otf):
        """
        Set the object temperature correction factor (SSE-related, unitless).
        """
        assert otf <= 127 * self.otf_unit and\
               otf >= -127 * self.otf_unit
        n = int(otf / self.otf_unit)
        if n < 0:
            regval = 256 - abs(n)
        else:
            regval = n
        self.log(logging.DEBUG, f'Setting OTF: {otf}, regvalue: {regval}')
        self.regwrite(MI48_OBJ_TEMP_FACTOR, regval)
        return None

    def set_compensation_parameter(self, address, value):
        """
        Encode a parameter `value` into 16-bin float (signed) and perform the
        necessaray 2-register writes, in order to convey it to the MI48.

        The `address` is either a valid name or the register address of the
        Least Significant Byte (LSB) that stores the parameter value.
        """
        lsb, msb = [b for b in np.float16(value).tobytes()]
        try:
            addr = int(address)
        except ValueError:
            try:
                ...
                addr = regmap[address]
        self.regwrite(addr, lsb)
        self.regwrite(addr+1, msb)


    def get_senxor_type(self):
    ...
    def parse_frame_header(self, header: list):
        ...
        Assume header is already a list of 16 bit unsigned int
        """
        result = {}
        ...
        result['iplock1']               = header[10]
        result['iplock2']               = header[11]
        # uncompensated frame statistics for SSEC
        result['ucMean']    = header[SPIHDR_UCMEAN] / 10. + KELVIN_0
        result['ucMiddle']  = header[SPIHDR_UCMIDDLE] / 10. + KELVIN_0
        result['ucMin']     = header[SPIHDR_UCMIN] / 10. + KELVIN_0
        result['ucMax']     = header[SPIHDR_UCMAX] / 10. + KELVIN_0
        result['ucMiddle_calc']  = 0.5 * (result['ucMin'] + result['ucMax'])
        result['ucUnmasked']  = header[SPIHDR_NUNMASKED]
        result['ucNgtmean']   = header[SPIHDR_NGTMEAN]
        result['ucNgtmiddle'] = header[SPIHDR_NGTMIDDLE]
        result['ucNorm']    = float(np.frombuffer(int(header[SPIHDR_NORM]).\
                                                      to_bytes(2, 'little'),
                                                  dtype=np.float16, count=1))
        result['ucRho'] = float(np.frombuffer(int(header[SPIHDR_RHO]).\
                                                      to_bytes(2, 'little'),
                                                  dtype=np.float16, count=1))
        # we must avoid division by zero, which happens if no compensation is used
        # both Ngtm* are zero if that is the case
        if result['ucNgtmean'] != 0:
            result['ucNorm_calc'] = (result['ucMean'] - result['ucMiddle_calc']) /\
                                    (result['ucMax'] - result['ucMin'])
            if result['ucNorm_calc'] > 0:
                result['ucRho_calc'] = (result['ucUnmasked'] - result['ucNgtmean']) /\
                                    (result['ucUnmasked'] - result['ucNgtmiddle'])
            else:
                result['ucRho_calc'] = (result['ucNgtmean']) /\
                                    (result['ucNgtmiddle'])
        else:
            result['ucRho_calc'] = 0.
        return result
    ...
    def start(self, stream=True, with_header=True):
        ...
        if self.data_type == 'raw_adc':
            # set DATA_TYPE (bit 7)
            mode = mode | DATA_TYPE_RAW_ADC
        else:
            # clear DATA_TYPE (bit 7)
            mode = mode & 0x7F
    ...
    def __repr__(self):
        ...
        _s.append('Camera Type {} (type {}), resolution {}, max FPS {}'.
                  format(self.senxor_name, self.senxor_type,
...
def format_header(hdr):
    ...
    s += f", Tmin {hdr['pixel_min']:.1f}, Tmax {hdr['pixel_max']:.1f}"
    s += f", ucMin {hdr['ucMin']:.1f}, ucMax {hdr['ucMax']:.1f}"
    s += f", ucMean {hdr['ucMean']:.1f}, ucMiddle {hdr['ucMiddle']:.1f}"
    #s += f", ucMean {hdr['ucMean']:.1f}, ucMiddle {hdr['ucMiddle']:.1f}, ucMiddle_calc {hdr['ucMiddle_calc']:.1f}"

    s += f", ucNorm {hdr['ucNorm']:6.3f}, ucRho {hdr['ucRho_calc']:5.3f}"
    if hdr['ucNorm'] > 0:
        Nmu = hdr['ucUnmasked'] - hdr['ucNgtmean']
        Nxi = hdr['ucUnmasked'] - hdr['ucNgtmiddle']
    else:
        Nmu = hdr['ucNgtmean']
        Nxi = hdr['ucNgtmiddle']
    s += f", ucNmu {Nmu:4d}, ucNxi {Nxi:4d}"
    #s += f", ucNorm_calc {hdr['ucNorm_calc']:6.3f}, ucRho_calc {hdr['ucRho_calc']:5.2f}"
    #s += f", ucNorm {hdr['ucNorm']:6.3f}, ucNorm_calc {hdr['ucNorm_calc']:6.3f}"
    #s += f", ucRho {hdr['ucRho']:5.2f}, ucRho_calc {hdr['ucRho_calc']:5.2f}"
    #s += '\n'
    return s
